...
void util_free_turbulence_widget(t_zephyros_turbulence_widget **pcfg)
{
	...
	if (cfg != NULL) {
		...
		util_free_field_errcovmatrix(&cfg->edr13_ecm);
...
void util_prepare_turbulence_widget(t_zephyros_turbulence_widget *cfg)
{
	...
	t_util_turbulence_karmanspec *karmanspec = malloc(sizeof(t_util_turbulence_karmanspec));
	...
	if (cfg->type == 1) {
		...
		for (ix=0; ix<cfg->Nx; ix++) {
			for (iy=0; iy<cfg->Ny; iy++) {
				for (iz=0; iz<cfg->Nz; iz++) {
					//u
					cfg->u_fourier[ix][iy][iz]  = util_turbulence_mann1998_C(karmanspec, cfg->freqx[ix], cfg->freqy[iy], cfg->freqz[iz], 0, 0, cfg->Lx / cfg->Nx, cfg->Ly / cfg->Ny, cfg->Lz / cfg->Nz) * cfg->random_fourier_c0[ix][iy][iz];
					cfg->u_fourier[ix][iy][iz] += util_turbulence_mann1998_C(karmanspec, cfg->freqx[ix], cfg->freqy[iy], cfg->freqz[iz], 0, 1, cfg->Lx / cfg->Nx, cfg->Ly / cfg->Ny, cfg->Lz / cfg->Nz) * cfg->random_fourier_c1[ix][iy][iz];
					cfg->u_fourier[ix][iy][iz] += util_turbulence_mann1998_C(karmanspec, cfg->freqx[ix], cfg->freqy[iy], cfg->freqz[iz], 0, 2, cfg->Lx / cfg->Nx, cfg->Ly / cfg->Ny, cfg->Lz / cfg->Nz) * cfg->random_fourier_c2[ix][iy][iz];

					//v
					cfg->v_fourier[ix][iy][iz]  = util_turbulence_mann1998_C(karmanspec, cfg->freqx[ix], cfg->freqy[iy], cfg->freqz[iz], 1, 0, cfg->Lx / cfg->Nx, cfg->Ly / cfg->Ny, cfg->Lz / cfg->Nz) * cfg->random_fourier_c0[ix][iy][iz];
					cfg->v_fourier[ix][iy][iz] += util_turbulence_mann1998_C(karmanspec, cfg->freqx[ix], cfg->freqy[iy], cfg->freqz[iz], 1, 1, cfg->Lx / cfg->Nx, cfg->Ly / cfg->Ny, cfg->Lz / cfg->Nz) * cfg->random_fourier_c1[ix][iy][iz];
					cfg->v_fourier[ix][iy][iz] += util_turbulence_mann1998_C(karmanspec, cfg->freqx[ix], cfg->freqy[iy], cfg->freqz[iz], 1, 2, cfg->Lx / cfg->Nx, cfg->Ly / cfg->Ny, cfg->Lz / cfg->Nz) * cfg->random_fourier_c2[ix][iy][iz];

					//w
					cfg->w_fourier[ix][iy][iz]  = util_turbulence_mann1998_C(karmanspec, cfg->freqx[ix], cfg->freqy[iy], cfg->freqz[iz], 2, 0, cfg->Lx / cfg->Nx, cfg->Ly / cfg->Ny, cfg->Lz / cfg->Nz) * cfg->random_fourier_c0[ix][iy][iz];
					cfg->w_fourier[ix][iy][iz] += util_turbulence_mann1998_C(karmanspec, cfg->freqx[ix], cfg->freqy[iy], cfg->freqz[iz], 2, 1, cfg->Lx / cfg->Nx, cfg->Ly / cfg->Ny, cfg->Lz / cfg->Nz) * cfg->random_fourier_c1[ix][iy][iz];
					cfg->w_fourier[ix][iy][iz] += util_turbulence_mann1998_C(karmanspec, cfg->freqx[ix], cfg->freqy[iy], cfg->freqz[iz], 2, 2, cfg->Lx / cfg->Nx, cfg->Ly / cfg->Ny, cfg->Lz / cfg->Nz) * cfg->random_fourier_c2[ix][iy][iz];
				}
			}
		}
			...
			for (iz=0; iz<cfg->Nz; iz++) {
				for (iy=0; iy<cfg->Ny; iy++) {
					for (ix=0; ix<cfg->Nx; ix++) {						
						...
						tmpxyzt[3] = 0.;
						util_turbulence_mann1998_uvw(cfg, tmpxyzt, 0, cfg->turb_u + i, 0, dummy);
						util_turbulence_mann1998_uvw(cfg, tmpxyzt, 1, cfg->turb_v + i, 0, dummy);
						util_turbulence_mann1998_uvw(cfg, tmpxyzt, 2, cfg->turb_w + i, 0, dummy);
					}
				}
			}
			free(tmpxyzt);
	...
	if (cfg->type == 5) {	//parametric turbulence
		...
		if (cfg->fit_edr13) {
			...
			util_prepare_field_errcovmatrix(cfg->field, cfg->grid_edr13_err, cfg->edr13_ecm);
		}
	}
	...
	double *uvw_derivatives)
{
	...
	if (cfg->type == 1) {
		//mann1998
		util_turbulence_mann1998_uvw(cfg, xyzt, i, ans, uvw_calcderivatives, uvw_derivatives);
	}
	if (cfg->type == 2) {
		//ctm
		util_turbulence_ctm_uvw(cfg, xyzt, i, ans, uvw_calcderivatives, uvw_derivatives);
	}
	if (cfg->type == 3) {
		//careta93
		util_turbulence_careta93_uvw(cfg, xyzt, i, ans, uvw_calcderivatives, uvw_derivatives);
	}
	if (cfg->type == 4) {
		//pinsky2006
		util_turbulence_pinsky2006_uvw(cfg, xyzt, i, ans, uvw_calcderivatives, uvw_derivatives);
	}
	...
	double *uvw_derivatives)
{
	...
	double complex phase;
	double complex *amp;
	t_util_turbulence_karmanspec *karmanspec; 
	int calcderivatives, bilint_special;
	double x = xyzt[0];
	double y = xyzt[1];
	double z = xyzt[2];
	...
	for (j = 0; j < jmax; j++) {
		...
		if (i == 0) {
			...
			for (ix=0; ix<cfg->Nx; ix++) {		 
				...
				amp = util_turbulence_ctm_C(karmanspec, fabs(cfg->freqx[ix]) / fct, cfg->Nx, cfg->Lx) * cfg->random_fourier_cx[ix];
				*ans += creal(amp * cexp(phase));
			}
		}
		if (i == 1) {
			...
			for (iy=0; iy<cfg->Ny; iy++) {		 
				...
				amp = util_turbulence_ctm_C(karmanspec, fabs(cfg->freqy[iy]) / fct, cfg->Ny, cfg->Ly) * cfg->random_fourier_cy[iy];
				*ans += creal(amp * cexp(phase));
			}
		}
		if (i == 2) {
			...
			for (iz=0; iz<cfg->Nz; iz++) {		 
				...
				amp = util_turbulence_ctm_C(karmanspec, fabs(cfg->freqz[iz]) / fct, cfg->Nz, cfg->Lz) * cfg->random_fourier_cz[iz];
				*ans += creal(amp * cexp(phase));
			}
		}
	}
	...
	double *uvw_derivatives)
{
	...
	*ans = 0.;
	...
	for (ix=0; ix<cfg->Nx; ix++) {
		for (iy=0; iy<cfg->Ny; iy++) {
			amp = coef * util_turbulence_careta1993_Q(cfg, cfg->freqx[ix], cfg->freqy[iy]) * cfg->random_fourier_beta[ix][iy];
...
double util_turbulence_mann1998_C(
	...
	double deltaz)
{
    return (pow(2. * M_PI,3) * (1. / deltax) * (1. / deltay) * (1. / deltaz)) * util_turbulence_mann1998_A(sp, kx, ky, kz, i, j);
}
	...
	int j)
{
    ...
    Ek = util_turbulence_mann1998_Ek(sp, kabs);
...
void util_turbulence_calibrate(t_zephyros_turbulence_widget *cfg)
{
	...
	for (i = 0; i < n; i++) {
		...
		util_turbulence_uvw(cfg, xyzt, cfg->calibration_dir, u + i, 0, dummy);
	}
