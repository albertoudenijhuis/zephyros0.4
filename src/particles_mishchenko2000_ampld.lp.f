c~ This test result was calculated by the code in its
c~ curent setting.
c~ 
c~ ICHOICE=1  NCHECK=1
c~ RAT= .963711
c~ PROLATE SPHEROIDS, A/B=   .5000000
c~ LAM=  6.283185   MRR= .1500D+01   MRI= .2000D-01
c~ ACCURACY OF COMPUTATIONS DDELT =  .10D-02
c~ EQUAL-SURFACE-AREA-SPHERE RADIUS= 10.0000
c~ thet0= 56.00  thet= 65.00  phi0=114.00  phi=128.00  alpha=145.00  beta= 52.00
c~ AMPLITUDE MATRIX
c~ S11=-.50941D+01 + i* .24402D+02
c~ S12=-.19425D+01 + i* .19971D+01
c~ S21=-.11521D+01 + i*-.30977D+01
c~ S22=-.69323D+01 + i* .24748D+02
c~ PHASE MATRIX
c~   650.3172  -17.9846   10.0498  -12.7580
c~   -21.1462  631.6322 -127.3059   87.2144
c~     6.8322  132.6131  635.2767  -34.7730
c~    -9.6629  -78.1229   51.4094  643.1738
c~  time =     .03 min
...
C   ANY DAMAGES THAT MAY RESULT FROM THE USE OF THE PROGRAM. 
      ...
      SUBROUTINE CALC_TMATRIX() 
      IMPLICIT REAL*8 (A-H,O-Z)
      INCLUDE 'particles_mishchenko2000_ampld.par.f'
...
C     Input output to communicate with c-interface
      REAL*8 AXI, RAT, LAM, MRR, MRI, EPS, DDELT
      REAL*8 ALPHA, BETA, THET0, THET, PHI0, PHI
      INTEGER NP, NDGS
      REAL*8 S11R, S11I, S12R, S12I, S21R, S21I, S22R, S22I
      REAL*8 Z11, Z12, Z13, Z14, Z21, Z22, Z23, Z24
      REAL*8 Z31, Z32, Z33, Z34, Z41, Z42, Z43, Z44
      INTEGER TMATRIXNR, NMAX
      COMMON /MISHCHENKO_IO/ AXI, RAT, LAM,
     &     MRR, MRI, EPS, NP, NDGS, DDELT,
...
c~       IF (WALB.GT.1D0+DDELT) PRINT 9111
 9111 FORMAT ('WARNING: W IS GREATER THAN 1')
      ...
      SUBROUTINE CALC_AMP_PHASE_MATRICES()
      IMPLICIT REAL*8 (A-H,O-Z)
